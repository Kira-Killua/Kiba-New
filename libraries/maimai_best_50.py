...
class Draw:
    ...
    async def whiledraw(self, data: Union[List[ChartInfo], List[PlayInfoDefault], List[PlayInfoDev]], best: bool, height: int = 0) -> None:
        ...
        for num, info in enumerate(data):
            ...
            self._im.alpha_composite(rate, (x + 300, y + 50))
            if info.fc:
            ...
            self._tb.draw(x + 155, y + 70, 28, f'{info.achievements:.4f}%', TEXT_COLOR[info.level_index], anchor='lm')
            self._tb.draw(x + 193, y + 132, 16, f'{info.dxScore}/{dxscore}', TEXT_COLOR_Base[info.level_index], anchor='mm')
            self._tb.draw(x + 155, y + 112, 16, f'{info.ds} -> {info.ra}', TEXT_COLOR_Base[info.level_index], anchor='lm')
