...
class Draw:
    ...
    async def whiledraw(self, data: Union[List[ChartInfo], List[PlayInfoDefault], List[PlayInfoDev]], best: bool, height: int = 0) -> None:
        ...
        TEXT_COLOR_Base = [(14, 117, 54, 255), (239, 164, 7, 255), (192, 32, 56, 255), (103, 20, 141, 255), (138, 0, 226, 255)]
        x = 70
        for num, info in enumerate(data):
            ...
            if info.rate.islower():
                rate = Image.open(maimaidir / f'UI_TTR_Rank_{score_Rank_l[info.rate]}.png').resize((100, 50))
            else:
                rate = Image.open(maimaidir / f'UI_TTR_Rank_{info.rate}.png').resize((100, 50))
            ...
            self._im.alpha_composite(rate, (x + 290, y + 50))
            if info.fc:
            ...
            title = info.title
            if coloumWidth(title) > 15:
                title = changeColumnWidth(title, 14) + '...'
            self._sy.draw(x + 150, y + 47, 17, title, TEXT_COLOR[info.level_index], anchor='lm')
            self._tb.draw(x + 150, y + 75, 28, f'{info.achievements:.4f}%', TEXT_COLOR[info.level_index], anchor='lm')
            self._tb.draw(x + 193, y + 133, 16, f'{info.dxScore}/{dxscore}', TEXT_COLOR_Base[info.level_index], anchor='mm')
            self._tb.draw(x + 155, y + 113, 16, f'{info.ds} -> {info.ra}', TEXT_COLOR_Base[info.level_index], anchor='lm')
...
class DrawBest(Draw):
    ...
    async def draw(self) -> Image.Image:
        ...
        self._im.alpha_composite(plate, (55, 100))
        icon = Image.open(maimaidir / 'UI_Icon_309503.png').resize((214, 214))
        self._im.alpha_composite(icon, (63, 108))
        if self.qqId:
            try:
                qqLogo = Image.open(BytesIO(await maiApi.qqlogo(self.qqId)))
                self._im.alpha_composite(Image.new('RGBA', (203, 203), (255, 255, 255, 255)), (70, 115))
                self._im.alpha_composite(qqLogo.convert('RGBA').resize((201, 201)), (70, 115))
            except Exception:
                pass
        self._im.alpha_composite(dx_rating, (290, 122))
        Rating = f'{self.Rating:05d}'
        for n, i in enumerate(Rating):
            self._im.alpha_composite(Image.open(maimaidir / f'UI_NUM_Drating_{i}.png').resize((28, 34)), (430 + 23 * n, 137))
        self._im.alpha_composite(Name, (295, 195))
        self._im.alpha_composite(MatchLevel, (610, 205))
        self._im.alpha_composite(ClassLevel, (601, 100))
        self._im.alpha_composite(rating, (295, 270))

        self._sy.draw(308, 228, 40, self.userName, (0, 0, 0, 255), 'lm')
        sdrating, dxrating = sum([_.ra for _ in self.sdBest]), sum([_.ra for _ in self.dxBest])
        self._tb.draw(534, 290, 28, f'B35: {sdrating} | B15: {dxrating}', (0, 0, 0, 255), 'mm', 3, (255, 255, 255, 255))
        self._tb.draw(1980, 200, 28, f'{datetime.datetime.today().strftime("%Y-%m-%d | %H:%M:%S")}', (0, 0, 0, 255), 'mm', 3, (255, 255, 255, 255))
        #self._mr.draw(900, 2465, 35, f'Feature by Yuri-YuzuChaN & BlueDeer233 | Generated by KibaBot New', (0, 50, 100, 255), 'mm', 3, (255, 255, 255, 255))
