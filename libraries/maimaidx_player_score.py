...
class DrawPlan(Draw):
    ...
    async def draw_plan(
        ...
        self._sy.draw(1100, 105, 30, f'已完成 {len(completed)} 个', (247, 75, 75, 255), 'mm')
        await self.whiledraw(completed[:30], True, 200)

        self._im.alpha_composite(self.title_bg, (800, 280 + clen))
        self._sy.draw(1100, 335 + clen, 30, f'未完成 {len(unfinished)} 个', (247, 75, 75, 255), 'mm')
        await self.whiledraw(unfinished[:30], True, 430 + clen)

        self._im.alpha_composite(self.title_bg, (800, 510 + clen + ulen))
        self._sy.draw(1100, 565 + clen + ulen, 30, f'未游玩 {len(notstarted)} 个', (247, 75, 75, 255), 'mm')
