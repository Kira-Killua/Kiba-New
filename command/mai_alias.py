...
alias_switch_off    = sv.on_prefix(['全局关闭别名推送'])
alias_switch_on     = sv.on_prefix(['全局开启别名推送'])
...
async def _(bot: NoneBot, ev: CQEvent):
    ...
    await bot.send(ev, '▼ 别名推送设置\n已全局关闭 Maimai 别名推送。将不再接收别名推送信息。')
...
async def _(bot: NoneBot, ev: CQEvent):
    try:
        ...
        id, alias_name = args
        if not mai.total_list.by_id(id):
            await bot.finish(ev, f'▼ 请注意\n未找到ID为 [{id}] 的曲目......真的有这个歌曲吗？')
        isexist = await maiApi.get_songs_alias(id)
        if alias_name in isexist['Alias']:
            await bot.finish(ev, f'▼ 请注意\n该曲目的别名 <{alias_name}> 已存在，不能重复添加别名，如果未生效，请联系BOT管理员使用指令 <更新别名库>。')
...
async def _(bot: NoneBot, ev: CQEvent):
    ...
    if findid and name.isdigit():
        ...
        if not alias_id:
            await bot.finish(ev, '▼ 请注意\n喔，我没有找到此歌曲......\n或许，您可以使用「添加别名」指令给该乐曲添加别名来试试？', at_sender=True)
        else:
            alias = alias_id
    else:
    ...
    if len(alias) != 1:
        ...
        await bot.finish(ev, f'▼ 别名重复\n找到{len(alias)}个相同别名的曲目：\n' + '\n======\n'.join(msg), at_sender=True)
    
    if len(alias[0].Alias) == 1:
        await bot.finish(ev, '▼ 请注意\n该曲目......天呐，它没有别名。', at_sender=True)
