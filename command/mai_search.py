...
async def _(bot: NoneBot, ev: CQEvent):
    ...
    if len(result) == 0:
        ...
        await bot.send(ev, f'▼ 请注意\n结果太多啦！一共检索到 {len(result)} 条，还请缩小查询范围之后再试试吧。', at_sender=True)
...
async def search_dx_song_bpm(bot: NoneBot, ev: CQEvent):
    if str(ev.group_id) in guess.Group:
        await bot.finish(ev, '▼ 在猜歌吧？\n本群现在正在猜歌，还请不要作弊哦~', at_sender=True)
    args = ev.message.extract_plain_text().strip().split()
    page = 1
    if len(args) == 1:
...
async def search_dx_song_artist(bot: NoneBot, ev: CQEvent):
    ...
    if len(args) == 1:
        ...
        if args[1].isdigit():
            ...
            await bot.finish(ev, '▼ 曲师查歌\n命令格式为：曲师查歌 <曲师名称> (<页数>)', at_sender=True)
    else:
        await bot.finish(ev, '▼ 曲师查歌\n命令格式为：曲师查歌 <曲师名称> (<页数>)', at_sender=True)
    if not name:
        return
...
async def search_dx_song_charter(bot: NoneBot, ev: CQEvent):
    ...
    if len(args) == 1:
        ...
        if args[1].isdigit():
            ...
            await bot.finish(ev, '▼ 谱师查歌\n命令格式为：谱师查歌 <谱师名称> (<页数>)', at_sender=True)
    else:
        await bot.finish(ev, '▼ 谱师查歌\n命令格式为：谱师查歌 <谱师名称> (<页数>)', at_sender=True)
    if not name:
        return
...
async def _(bot: NoneBot, ev: CQEvent):
    ...
    if alias_data:
        if len(alias_data) != 1:
            ...
            await bot.finish(ev, '▼ 别名查歌\n我猜，您要找的是不是：' + (await draw_music_info(music, ev.user_id)), at_sender=True)
    # id
    if name.isdigit() and (music := mai.total_list.by_id(name)):
        await bot.finish(ev, '▼ ID 查歌\n我猜，您要找的是不是：' + (await draw_music_info(music, ev.user_id)), at_sender=True)
    ...
    if len(result) == 0:
        ...
        msg = await draw_music_info(result.random(), ev.user_id)
        await bot.finish(ev, '▼ 别名查歌\n您输入的可能为歌曲标题。您要找的是不是：' + await draw_music_info(result.random(), ev.user_id), at_sender=True)
    elif len(result) < 50:
        msg = f'▼ 请注意\n未找到别名为「{name}」的歌曲，但找到{len(result)}个相似标题的曲目：\n'
        for music in sorted(result, key=lambda x: int(x.id)):
            msg += f'{music.id}. {music.title}\n'
        msg += '※ 请使用「id xxxxx」查询指定曲目'
        await bot.finish(ev, msg.strip(), at_sender=True)
    else:
        await bot.finish(ev, f'▼ 别名查歌\n结果太多啦！一共有 {len(result)} 条，还请缩小查询范围。', at_sender=True)
