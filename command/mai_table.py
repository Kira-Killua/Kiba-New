...
async def _(bot: NoneBot, ev: CQEvent):
    ...
    if rating:
        ...
        if ver in platecn:
            ver = platecn[ver]
        if ver in ['舞', '霸']:
            await bot.finish(ev, '▼ 牌子查询\n现在还暂不支持查询「舞」系和「霸者」的牌子......', at_sender=True)
        if f'{ver}{plan}' == '真将':
            await bot.finish(ev, '▼ 牌子查询\n真系没有真将哦', at_sender=True)
        pic = await draw_plate_table(qqid, ver, plan)
        await bot.send(ev, pic, at_sender=True)
    else:
        await bot.send(ev, '▼ 请注意\n无法识别的表格', at_sender=True)
...
async def _(bot: NoneBot, ev: CQEvent):
    args: str = ev.message.extract_plain_text().strip()
    if args in levelList[:5]:
        await bot.send(ev, '▼ 定数表\n只支持查询lv6-15的定数表', at_sender=True)
        ...
        await bot.send(ev, '▼ 定数表\n无法识别的定数。检查一下输入看看。', at_sender=True)
...
async def _(bot: NoneBot, ev: CQEvent):
    ...
    if rating and rating not in levelList:
        await bot.finish(ev, '▼ 请注意\n这个rating......貌似是没有对应等级。', at_sender=True)
    elif match.group(3):
...
async def _(bot: NoneBot, ev: CQEvent):
    ...
    if category:
        if category in ['已完成', '未完成', '未开始']:
            ...
            await bot.finish(ev, f'▼ 请注意\n无法指定查询「{category}」', at_sender=True)
    else:
        category = 'default'
